...
public class GlowPadView extends View {
	...
	//peter.zhang add code
	//update by berrytao
	//berrytao BEGIN
	//TargetDrawable mAnswerFoucsBg;
	//TargetDrawable mDeclineFoucsBg;
	//TargetDrawable mTextFoucsBg;
	//berrytao add code
	//TargetDrawable mMuteFouchBg;

	//guang quan
	//TargetDrawable mGuangquan1;
	//TargetDrawable mGuangquan2;
	//TargetDrawable mGuangquan3;
	//TargetDrawable mGuangquan4;
	TargetDrawable mGuangquan;	
//	TargetDrawable mLoading1;
//	TargetDrawable mLoading2;
//	TargetDrawable mLoading3;

	
	//private Tweener mGuangquanAnimator1;
	//private Tweener mGuangquanAnimator2;
	//private Tweener mGuangquanAnimator3;
	//private Tweener mGuangquanAnimator4;
//	private Tweener mGuangquanAnimator;
//	private Tweener mLoadAnimator1;
//	private Tweener mLoadAnimator2;
//	private Tweener mLoadAnimator3;
	
	private Tweener mAnswerAmin;

	private float mLoad1MarginCenter;	
	private float mLoad2MarginCenter;	
	private float mLoad3MarginCenter;	
	//berrytao END
	private float mTargetPositionRight;	
    ...
    private void doFinish() {
        ...
        if(mActiveTarget != -1){
        	mTargetDrawables.get(mActiveTarget).setState(TargetDrawable.STATE_ACTIVE);
        }
    }
    ...
    private void showTargets(boolean animate) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            target.setState(TargetDrawable.STATE_ACTIVE);
...
//        }
        ...
        Log.v(TAG, "1023 mActiveTarget ="+mActiveTarget+" activeTarget="+activeTarget);
        mActiveTarget = activeTarget;
        
        if(mActiveTarget != -1){
        	mTargetDrawables.get(mActiveTarget).setState(TargetDrawable.STATE_PRESSED);
        }
    }
    ...
    private boolean trySwitchToFirstTouchState(float x, float y) {
        ...
        if (DEBUG) Log.v(TAG, "** mAlwaysTrackFinger:"+mAlwaysTrackFinger+"X+Y"+dist2(tx, ty)+",Z:"+getScaledGlowRadiusSquared());//add by berrytao
        if (mAlwaysTrackFinger || dist2(tx,ty) <= getScaledGlowRadiusSquared()) {
            if (DEBUG) Log.v(TAG, "** Handle HIT");
            switchToState(STATE_FIRST_TOUCH, x, y);
            updateGlowPosition(tx, ty);
			moveHandleTo(x, y, false);//peter.zhang add 
            mDragging = true;
            return true;
        }
        return false;
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		//mOuterRing.setScaleX(0.8f);
		//mOuterRing.setScaleY(0.8f);

		Log.d("TEST","newWaveCenterX="+newWaveCenterX);
		Log.d("TEST","newWaveCenterY="+newWaveCenterY);
